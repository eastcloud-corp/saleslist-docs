...
| --- | --- | --- | --- |
| 国税庁法人番号 Web-API | 商号・所在地・法人番号・設立年月日 | 毎晩自動実行。直近30日以内に取得済みの企業はスキップ。1日あたり約4,000件まで。`CORPORATE_NUMBER_API_TOKEN` が必須。 | 手動実行ボタンは開発用。実運用では深夜バッチに任せる。 |
| 自治体オープンデータ（東京都・静岡県・大阪府） | 都道府県・市区町村 / 代表電話番号 / 資本金 / 業種分類コード | 毎晩自動実行。ソースキー未指定時は YAML に定義した全ソースを順番に処理。 | 必要に応じて管理者が `source_keys` を指定して個別再取得できる。 |
| Facebook Page メトリクス | フォロワー数・最新投稿日時 | 将来的に稼働予定。`FACEBOOK_ACCESS_TOKEN` 未設定時はスキップし、履歴にもその旨が記録される。 | 本番導入まで「準備中」と案内する。 |
| AI 補完（PowerPlexy） | 担当者氏名・担当者役職・設立年・資本金・事業概要 | 手動トリガーまたは必要に応じて Celery Beat（`ai.enrich`）。1 回あたり最大 500 件、月間 5,000 件 / 約 150 USD（従量課金上限）。 | `POWERPLEXY_*` 環境変数で API キーと上限を管理。 |

- **取得 / 投入 の見方**:
  - 「取得」はジョブが調査対象とした企業件数です。
  - 「投入」はレビュー待ちテーブルに追加された候補件数です。0 の場合は、対象企業をチェックしたものの AI/ルールの補完値が見つからず、レビューには新規候補が表示されません。
...
4. **利用状況の確認**: AI 補完を選択すると、画面上部に「PowerPlexy 利用状況」カードが表示されます。月間コール数・コスト・残量を確認し、上限に近付いたら実行を控えてください。
5. **ログ参照**: 失敗（`FAILURE`）行で `metadata.result` に詳細ログが残るので、Celery ログと突合すると原因追跡が早いです。
6. **権限制御**: 一般ユーザーからはメニューごと非表示。管理者が必要なときのみ利用してください。

> ※ 次回実行予定およびジョブ別スケジュールに表示される時刻は JST（Asia/Tokyo）です。
